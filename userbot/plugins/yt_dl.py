...
async def yt_dl(event):
    ...
    if fmt:
        fmt = fmt.strip()
        if fmt == 'listformats':
            info = await extract_info(concurrent.futures.ThreadPoolExecutor(),
                                      params, url)
    ...
    output = await extract_info(concurrent.futures.ThreadPoolExecutor(),
                                params,
                                url,
                                download=True)
    warning = (
    ...
    await event.answer(result,
                       log=("YT_DL", f"Successfully downloaded {url}!"))
