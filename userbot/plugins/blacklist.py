...
async def unblacklister(event: NewMessage.Event) -> None:
    ...
    if response == 0:
        await event.answer(
            ...
            log=('blacklist',
                 f'Removed {option} blacklist: {value} for {key}.'))
    elif response == 1:
        await event.answer(
            f"__Skipped {option} blacklist, it doesn't exists for {key}.__",
            reply=True)
    else:
        await event.answer(f"__There are no blackists saved for {key}.__",
                           reply=True)
...
async def whitelister(event: NewMessage.Event) -> None:
    ...
    if user:
        ...
        await event.answer(
            ...
            log=('whitelist', f'Whitelisted user [{wl}](tg://user?id={wl}).'))
    elif chat:
        ...
        await event.answer(f"**Successfully whitelisted chat** {wl}",
                           log=('whitelist', f'Whitelisted chat {wl}.'))
    else:
...
async def unwhitelister(event: NewMessage.Event) -> None:
    ...
    if value in whitelistedUsers:
        ...
        await event.answer(f"__Removed chat {value} from whitelist.__",
                           log=('whitelist', f'Unwhitelisted chat {value}.'))
    else:
