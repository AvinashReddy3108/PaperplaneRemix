...
dbl_pattern = (r"r(?:e)?m(?:ove)?"
               ...
               r"b(?:lack)?l(?:ist)?"
               r"(?: |$)(?P<match>[\s\S]*)")
wl_pattern = (r"w(?:hite)?l(?:ist)?" r"(?: |$)(?P<match>[\s\S]*)")
...
bls_pattern = (r"(?P<global>g(?:lobal)?)?"
               ...
               r"(?: |$)"
               r"(?:(?P<option>\w+))?")
wls_pattern = (r"w(?:hite)?l(?:ist)?s" r"(?: |$)" r"(?:(?P<option>\w+))?")
bld_pattern = (r"blacklisted" r"(?: |$)" r"(?:(?P<option>\w+))?")
...
async def inc_listener(event: NewMessage.Event) -> None:
    ...
    if tgid_check:
        if (event.sender_id in getattr(GlobalBlacklist, 'tgid', []) +
        ...
        for entity in entities:
            if (isinstance(
                ...
                entity = id_pattern.search(
                    event.text[entity.offset:entity.offset +
                               entity.length]).group('e')
    ...
    if event.user_added or event.user_joined:
        ...
        if (chat_id in whitelistedChats or sender_id in whitelistedUsers
            ...
            return
              ...
              or (localbl and localbl.tgid and sender_id in localbl.tgid)):
